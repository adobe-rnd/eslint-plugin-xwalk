...
function getFieldNames(model, includeNested = false) {
  let fieldNames = [];
  for (const { name, component, multi, ...rest } of model.fields) {
    ...
    if (component === 'container') {
      ...
      if (includeNested) {
        const nestedFieldNames = getFieldNames(rest, true);
        fieldNames = [...fieldNames, ...nestedFieldNames.map(nestedName => `${name}/${nestedName}`)];
        continue;
      }
    }
